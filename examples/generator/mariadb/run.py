"""Run: build logical catalog → load into MariaDB."""
...
async def run(
    ...
    *,
    ...
    print(f"loaded mariadb {summary} movielens={catalog.meta.get('movielens')}")
    return 0


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Load RecQL demo catalog into MariaDB")
    p.add_argument("--database", required=True, help="mariadb://user:pass@host:3306/recql")
    p.add_argument(
        "--encode-backend",
        default="fake",
